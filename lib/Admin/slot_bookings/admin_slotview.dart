...
import 'package:intl/intl.dart';
import 'package:slotbookingadmin/Admin/navbar/adminNavbar.dart';
import 'package:slotbookingadmin/shared/providers/firebase_provider.dart';
import 'package:slotbookingadmin/theme/app_colors.dart';
                            ...
                            return Padding(
                              ...
                              child: _BookingCard(
                                ...
                                onDetails: () =>
                                    _showDetailsSheet(context, doc.id, data),
                                onDelete: () =>
                                    _deleteSlot(context, doc.id, data),
                              ),
                            );
                          }),
                      ],
          ...
          actions: [
            ...
            ElevatedButton(
              onPressed: () async {
                ...
                try {
                  if (status == 'cancelled') {
                    // ✅ Cancel flow — user_bookings + notification bhi update hoga
                    await FirebaseBookingService.cancelBooking(
                      bookingId: bookingId,
                      bookingData: data,
                    );
                    _showSnack(
                      'Booking cancelled. User ko notify kar diya gaya.',
                      Colors.orange[700]!,
                    );
                  } else {
                    // ✅ Other status update
                    await FirebaseBookingService.updateBookingStatus(
                      bookingId: bookingId,
                      newStatus: status,
                      bookingData: data,
                    );
                    _showSnack('Status update ho gaya.', AppColors.primary);
                  }
                } catch (e) {
              ...
              child: Text(status == 'cancelled' ? 'Cancel Booking' : 'Save'),
            ),
          ],
        ),
      ),
      ...
      builder: (ctx) => Padding(
        ...
        child: Column(
          ...
          children: [
            ...
            // ✅ Cancel button directly from details sheet
              ...
              Padding(
                ...
                child: SizedBox(
                  ...
                  child: OutlinedButton.icon(
                    onPressed: () async {
                      Navigator.pop(ctx);
                      await _confirmAndCancelBooking(context, bookingId, data);
                    },
                    icon: const Icon(
  ...
  Future<void> _confirmAndCancelBooking(
    ...
    final confirmed = await showDialog<bool>(
      ...
      builder: (ctx) => AlertDialog(
        ...
        content: const Text(
          'Kya aap sure hain? User ko notification mil jayegi aur unki booking bhi cancel ho jayegi.',
          style: TextStyle(fontSize: 14, height: 1.5),
        ),
        actions: [
          TextButton(
            onPressed: () => Navigator.pop(ctx, false),
            child: Text('Nahi', style: TextStyle(color: Colors.grey[600])),
          ),
          ElevatedButton(
    ...
    try {
      // ✅ Ek function call — teen kaam hote hain:
      //    1. admin_bookings → cancelled
      //    2. user_bookings  → cancelled
      //    3. notifications  → user ko in-app notification
      await FirebaseBookingService.cancelBooking(
        bookingId: bookingId,
        bookingData: data,
      );
      ...
      if (mounted) {
        _showSnack(
          'Booking cancel ho gayi. User ko notification bhej di gayi.',
          Colors.orange[700]!,
        );
      }
    } catch (e) {
      if (mounted) {
        _showSnack('Error aaya: $e', Colors.red[700]!);
      }
    }
  }
        ...
        actions: [
          ...
          ElevatedButton(
            onPressed: () async {
              Navigator.pop(ctx);
              try {
                // ✅ Delete + user notify (agar userId hai toh)
                await FirebaseBookingService.deleteSlot(
                  bookingId: bookingId,
                  bookingData: data,
                );
                if (mounted) {
                  _showSnack('Slot delete ho gaya.', Colors.red[700]!);
                }
              } catch (e) {
                if (mounted) {
                  _showSnack('Error: $e', Colors.red[700]!);
